...
echo '<a href="'. $path .'beminder_sell_buy_admin_dashboard.php">Home</a><br/>';
 if($num==3){
	 ...
	 echo "<hr/>";
	 $all_orders=$order->find_order_by_uploader($conditions[1]);
	ordershow($all_orders,$path);
 }else {
	 $all_orders=$order->find_all();
	 ordershow($all_orders,$path);
	...
	?>
		...
		foreach($all_orders as $order){
			echo '<tr>
					...
					<th>'.$order->status.'</th>
					<th>'.$order->ofor.' </th>
					<th> <a href="'.$path.'beminder_admin_orders.php/'.$order->too.'">'.$order->too.'</a></th>
				</tr>';
