 ...
 if($num == 3){
	$allbooks=$sbooks->find_by_subtype($conditions[2]);
	booksshow($allbooks,$path);
 }else if($num == 2){
	 $allbooks=$sbooks->find_by_type($conditions[1]);
	 booksshow($allbooks,$path);
 }else {
	 $allbooks=$sbooks->find_all();
	 booksshow($allbooks,$path);
 } 
 ...
 <div>
 	<a href="beminder_sell_buy_seller_dashboard.php">Home</a>
 ...
 <tr>
    ...
    <th><a href="<?php echo $path; ?>beminder_seller_books.php/<?php echo $onebook->type ?>"><?php echo sbooks::$types[$onebook->type]; ?></a></th>
    <th><a href="<?php echo $path; ?>beminder_seller_books.php/<?php echo $onebook->type ?>/<?php echo $onebook->subtype ?>"><?php echo sbooks::$subtypes[$onebook->subtype]; ?></a></th>
    <th><?php echo $onebook->lasttype; ?></th>
 	<th><?php echo $onebook->ISBN1; ?></th>
    <th>
    <form action="beminder_seller_books.php" method="post">
    	<input type="hidden" name="id" value="<?php echo $onebook->id; ?>">
    	<input type="submit" value="delete" name="delete">
    </form>
    </th>
    <th>
    <form action="beminder_seller_books_add_update.php" method="post">
    	<input type="hidden" name="id" value="<?php echo $onebook->id; ?>">
